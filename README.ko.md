...
자주 보내는 사람은 이름으로 저장해두고 부를 수 있습니다.
...
- **Python 3.11 이상.** 터미널에서 `python --version`으로 확인합니다. (Windows에서는 `py
  ...
  받습니다.
- **NAVER나 Gmail 계정.** Gmail은 누구나 만들 수 있습니다. NAVER는 한국 휴대폰 번호가 있으면
  가장 쉽습니다 — 외국인은 외국인등록증(ARC)으로 번호를 개통합니다. 한국 번호가 없으면 여권·정부
  발급 신분증 인증으로도 가입할 수 있고, 이 경우 하루이틀 걸립니다. 요건은 자주 바뀌고 국가마다
  다르니 NAVER 가입 페이지에서 확인합니다. NAVER 계정이 없으면 Gmail로 하면 됩니다.
- **그 계정의 앱 비밀번호.** 3단계에서 받습니다. **평소 로그인하는 비밀번호로는 안 됩니다** —
  두 서비스 다 거부합니다.
...
홈 폴더 아래 `.config/mailmail/` 에 `config.toml` 파일을 만듭니다. 전체 경로는 이렇습니다.
...
**비밀번호는 이 파일에 넣지 않습니다.** 3단계에서 따로 저장합니다.

## 3. 앱 비밀번호 받기

**여기가 가장 많이 막히는 곳입니다.** 평소 로그인 비밀번호는 두 서비스 다 거부하고, 앱
비밀번호를 따로 발급받아야 합니다. 둘의 형식이 정반대라 헷갈리기 쉽습니다.
...
2025년 6월 24일부터 메일 프로그램 연결에 2단계 인증과 앱 비밀번호가 **필수**가 되었습니다. 예전에
잘 되던 설정이 갑자기 안 된다면 그 때문입니다.

1. **네이버ID → 보안설정 → 2단계 인증**을 켭니다. 이걸 안 켜면 다음 단계 메뉴가 아예 없습니다.
2. 같은 화면에서 **애플리케이션 비밀번호 → 생성하기**.
   - **종류선택은 그냥 이름표입니다.** 아웃룩·아이폰·지메일 중 뭘 고르든 나오는 비밀번호는 같습니다.
     직접 입력에 mailmail이라고 적어두면 나중에 알아보기 좋습니다.
   - 12자리 대문자+숫자가 나옵니다. **그 화면을 벗어나면 다시 못 봅니다.** 복사해둡니다.
3. **메일 → 환경설정 → POP3/IMAP 설정**에서 **SMTP 사용함**을 확인합니다. 이미 켜져 있어도
   **사용 안 함 → 저장 → 사용함 → 저장** 으로 한 번 껐다 켭니다. 2025년 6월 정책 변경이 이때
   반영됩니다.

### Gmail

1. **2단계 인증을 먼저 켭니다.** 안 켜면 앱 비밀번호 메뉴가 나타나지 않습니다.
2. [myaccount.google.com/apppasswords](https://myaccount.google.com/apppasswords) 에서
   발급합니다.
3. 16자리 소문자가 네 칸씩 띄어서 나옵니다. 공백은 있든 없든 상관없습니다.
...
메일 발송에만 쓰이고, 계정 비밀번호는 그대로 둔 채 언제든 취소할 수 있습니다. 계정 비밀번호는
넣지 않습니다.

### 잘 됐는지 확인

메일을 보내보지 않고도 로그인만 시험할 수 있습니다.
...
`OK`가 나오면 끝났습니다. 안 나오면:

| 서버가 하는 말 | 뜻 |
|---|---|
| `535 5.7.1 Username and Password not accepted` (NAVER) | 비밀번호가 틀렸거나, SMTP가 꺼져 있습니다. **둘 다 확인합니다** — 이 메시지는 구분해주지 않습니다 |
| `534 5.7.9 Application-specific password required` (Gmail) | 로그인 비밀번호를 넣었습니다. 앱 비밀번호로 다시 |
...
주소록에 등록했으면 이름으로 부를 수 있습니다. 주소와 섞어도 됩니다.
...
본문은 적은 그대로 나가므로 줄바꿈이 그대로 삽니다. 긴 본문은 따옴표 세 개가 편합니다.
...
- **`cc`를 안 적으면 참조는 비어 있습니다.** 적지 않은 것은 안 들어갑니다.
- **`bcc`는 받는 사람들에게 안 보입니다.** 숨은 참조끼리도 서로 모릅니다.
- **`html`과 `body`는 같은 내용이어야 합니다.** 메일 앱에 따라 둘 중 하나만 보이므로, 다르게
...
`Mail`은 `send`과 같은 어휘입니다 — `to`·`cc`·`bcc`에 주소나 주소록 이름을 섞어 쓰고,
`attachments`는 경로입니다. 개인화(이름·수치·첨부)는 통마다 `Mail`을 다르게 만들면 됩니다.

몇 가지 알아둘 것:

- **결과는 통 순서 그대로 목록으로 옵니다.** `zip(mails, receipts)`로 누가 어떻게 됐는지
  짝지을 수 있습니다. 한 통이 수신자 전원 거부돼도 그 통은 `accepted`가 빈 receipt로 남고
  **나머지는 그대로 나갑니다** — 30통 중 3통이 막히면 27통은 갑니다.
- **대부분의 나쁜 행은 아무것도 보내기 전에 막습니다.** 어느 통에 주소록에 없는 이름, 빈
  제목, 차단된 첨부, 또는 이미 한도를 넘는 첨부가 있으면 **연결을 열기 전에** 예외가 나고
  한 통도 안 나갑니다.
- **되돌릴 수 없는 두 경우:** (1) 어떤 통이 조립하고 나서야 크기 한도를 넘거나(사전 검사는
  첨부만 재고 완성된 MIME은 못 잽니다), (2) 보내는 도중 연결이 끊기면(네트워크 장애). 둘 다
...
`--to`(그리고 `--cc`, `--bcc`)는 주소나 주소록 이름을 받고, 여러 개면 반복해서 줍니다.
`--attach`도 반복됩니다. 줄바꿈이 있는 본문은 셸 인자 하나로 넘기기보다 파일이나 파이프로
주는 편이 낫습니다.
...
CSV로 여러 통 한 번에 — 한 행이 한 통, 배치 전체가 로그인 한 번입니다. 헤더가 필드 이름이고,
`to`·`subject`·`body`는 필수, `cc`·`bcc`·`html`·`attachments`는 선택입니다. 한 셀에 여러 개가
...
- **실행파일.** `.exe`, `.dll`, `.jar`, `.js`, `.bat`, `.vbs`, `.ps1`, `.msi` 같은 것들.
  `.zip`이나 `.tar.gz` **안에 넣어도 똑같이 막힙니다.** ([Gmail이 공개한 목록](https://support.google.com/mail/answer/6590).
  NAVER는 목록을 공개하지 않아 같은 기준을 적용합니다.)
- **비밀번호가 걸린 zip.** 안에 뭐가 들었든 거부됩니다. 서비스가 열어볼 수 없기 때문입니다.
- **너무 크거나 너무 여러 겹인 압축.** 4겹까지, 안쪽 파일 64MB까지만 들여다봅니다.

보내야 하면 링크로 공유합니다.

**크기 한도** — 첨부는 메일에 실리면서 약 37% 커집니다. 그래서 원본 파일 합계 기준으로
**Gmail 약 25MB**, **NAVER 약 27MB**가 상한입니다. 웹 화면이 안내하는 숫자와 다를 수 있는데,
실제로 받아주는 쪽은 이쪽입니다.

**`.7z`과 `.rar`은 안을 볼 수 없습니다.** 파이썬 표준 라이브러리가 이 형식을 못 읽습니다. 이 안에
...
그 다음부터는 Claude Code에서 그냥 말하면 됩니다.

> 6월 손익 요약해서 표로 만들고, balance.xlsx 첨부해서 lead 한테 보내줘.

**무엇을 본문에, 무엇을 첨부로, 누구에게** 를 짚어주면 그대로 조립됩니다. 안 짚은 것은 묻습니다.
추측하지 않습니다.

보내기 전에 **받는사람·제목·첨부를 펼쳐 보여주고 승인을 받습니다.** 주소록 이름은 실제 주소로
...
| | 뜻 |
|---|---|
| `ConfigError` | 설정 파일이 없거나 형식이 틀렸습니다. 2단계로 |
| `MissingPasswordError` | 비밀번호를 아직 저장하지 않았습니다. 3단계로 |
| `UnknownContactError` | 주소록에 없는 이름입니다. 아는 이름들을 함께 알려줍니다 |
| `BlockedAttachmentError` | 메일 서비스가 막는 파일입니다. 링크로 공유합니다 |
