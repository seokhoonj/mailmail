...
# 인자를 안 넘겼을 때 쓸 수신자. bcc 도 같은 모양으로 쓸 수 있다.
[defaults]
to = "lead"
cc = ["me"]
```
...
파이썬에서 `str`은 그 자체로 문자들의 이터러블이다. 문자열을 그대로 받아주면
`to="lead@example.com"`이 주소 하나가 아니라 **16명의 한 글자짜리 수신자**가 된다. 그래서
생성자는 거부하고, 문자열 지름길은 `compose`에 뒀다. `send_mail`도 문자열을 받아주므로,
평소 쓰는 자리에서는 이 구분이 드러나지 않는다.
...
`skills/send-mail/`에 이 패키지의 `send_mail(...)`을 호출하는 얇은 래퍼가 있다. "이 결과
메일로 보내줘" 같은 요청을 받으면 호출을 조립해 실행하고, 발송 전 수신자·제목·첨부를
